...
export default async function CourseChapters({ course }: CourseChaptersProps) {
  ...
  const accordionItems = await Promise.all(chapters.map(async (chapter) => {
    const chapterArticles = await getChapterArticles(chapter.id) as Article[];
        ...
        <ul className="space-y-2">
          {chapterArticles.length > 0 ? (
            chapterArticles.map((article: Article, index) => (
                <li 
                key={article.id} 
                className={`py-2 ${index < chapterArticles.length - 1 ? 'border-b' : ''}`}
                >
                <Link
